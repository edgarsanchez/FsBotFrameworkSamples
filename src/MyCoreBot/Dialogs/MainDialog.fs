...
type MainDialog (luisRecognizer: FlightBookingRecognizer, bookingDialog: BookingDialog, logger: ILogger<MainDialog>) as __ =
    ...
    let showWarningForUnsupportedCities (context: ITurnContext) (luisResult: FlightBooking) cancellationToken =
        ...
        if unsupportedCities.IsEmpty then
            ...
            let messageText = $"Sorry but the following airports are not supported: {SysString.Join(',', unsupportedCities)}"
            let message = MessageFactory.Text(messageText, messageText, InputHints.IgnoringInput)
            unitTask { return! context.SendActivityAsync(message, cancellationToken) }
        ...
        task {
            if luisRecognizer.IsConfigured then
                ...
                let messageText =
                    ...
                    | null ->
                        "What can I help you with today?\nSay something like \"Book a flight from Paris to Berlin on March 21, 2022\""
                    | options ->
                        options.ToString() 
        ...
        task {
            if luisRecognizer.IsConfigured then
                ...
                | _ ->
                    ...
                    let didntUnderstandMessageText = $"Sorry, I didn't get that. Please try asking in a different way (intent was {luisResult.TopIntent().intent})"
    ...
    let finalStepAsync (stepContext: WaterfallStepContext) cancellationToken =
        task {
            ...
            | :? BookingDetails as result -> 
                ...
                let travelDateMsg = timeProperty.ToNaturalLanguage System.DateTime.Now
                let messageText = $"I have you booked to {result.Destination} from {result.Origin} on {travelDateMsg}"
                let message = MessageFactory.Text(messageText, messageText, InputHints.IgnoringInput)
